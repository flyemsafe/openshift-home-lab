...
function generate_idm_dm_pwd () {
    ...
    then
        ...
        idm_dm_pwd=$(cat ${vaultfile}|awk '/idm_dm_pwd:/ {print $2;exit}')
    fi
...
function ask_user_for_custom_idm_server () {
    ...
    # Use ansible-vault view if vault file is encrypted otherwise use
    # the cat command to veiw it.
    if ansible-vault view "${vaultfile}" > /dev/null 2>&1
    then
        idm_ssh_user=$(cat ${vaultfile}|awk '/idm_ssh_user:/ {print $2;exit}')
        idm_dm_pwd=$(cat ${vaultfile}|awk '/idm_dm_pwd:/ {print $2;exit}')
        idm_admin_pwd=$(cat ${vaultfile}|awk '/idm_admin_pwd:/ {print $2;exit}')
    else
    ...
    ## Pull the current defined IdM vars values for the IdM user
    if [ -f ${vaultfile} ]
    then
        if [ "A${idm_dm_pwd}" == 'A""' ];
        then
    ...
    ## If all the IdM required vars are present
    ## tell the installer not to skip this function on the next run
...
function set_idm_static_ip () {
    ...
    then
        ...
        sed -i "s/qubinode_dns_server_ip:.*/qubinode_dns_server_ip: "$USER_IDM_SERVER_IP"/g" "${vars_file}"
